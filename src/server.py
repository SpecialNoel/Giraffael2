...
class Server:
    def __init__(self):
        ...
        self.server = self.init_server(self.SERVER_IP, self.SERVER_PORT) # server socket
    ...
    def init_server(self, serverIP, serverPort):
    ...
    def remove_a_client_from_clients_by_socket(self, client, clients):
    ...
    def check_username_validness(self, username, maxUsernameLength):
    ...
    def handle_client_username_message(self, client, maxUsernameLength):
    ...
    def check_room_code_validness(self, roomCode, roomCodes):
        # Check if the received room code exists in roomCodes
        return roomCode in roomCodes


    def handle_client_room_code_message(self, client, address, roomCodeLength, roomCodes):
    ...
    def generate_an_unique_room_code(self, roomCodeLength, roomCodes):
    ...
    def genrate_and_send_room_code(self, conn, address, roomCodeLength, roomCodes):
    ...
    def create_room(self, roomCode, rooms):
        room = Room(roomCode)
        rooms.append(room) 
        return


    def enter_room(self, clientObj, roomCode, rooms):
        room = [r for r in rooms if r.get_room_code() == roomCode][0]
        room.add_client_to_client_list(clientObj)
        return
        

    def check_room_exist_with_room_code(self, roomCode, rooms):
        for room in rooms:
    ...
    def handle_client_normal_message(self, client, decodedMsg, clients, 
                                            rooms, roomCode):
    ...
    def close_connection_with_client_when_error(self, e, client, clients, address,
    ...
    def handle_one_client(self, clientObj, clients, rooms, maxClientCount, roomCodes,
                        shutdownEvent):
    ...
    def start_handling_one_client(self, clientObj, clients, rooms, 
                                maxClientCount, roomCodes, shutdownEvent,
                                threads):
    ...
    def test_reach_max_client_count(self, clients, maxClientCount, conn, address):
    ...
    def accept_a_connection(self, server, clients, rooms, maxClientCount, 
                            roomCodeLength, maxUsernameLength,
                            roomCodes, shutdownEvent, threads):
        try:
    ...
    def run_server(self):
        ...
        threads = [] # all threads that handle each client
        ...
        while True:
            ...
            if not accept_a_connection(server, clients, rooms, MAX_CLIENT_COUNT, 
                                ROOM_CODE_LENGTH, MAX_USERNAME_LENGTH, 
                                roomCodes, shutdownEvent, threads):
                break
