...
def handle_room_decision(client, msgContentSize):
    ...
    msg, response = send_decision_on_room(client, msgContentSize)
    while msg.upper() != 'C':
        print(f'msg: [{msg}], response from server: [{response}]')
        if msg.upper() == 'E':
            msg, response = send_room_code(client, msgContentSize)
            print(f'msg: [{msg}], response from server: [{response}]')
            if msg.upper() == 'C':
                break
